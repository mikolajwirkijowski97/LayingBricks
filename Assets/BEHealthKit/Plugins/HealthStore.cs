	...
	public partial class HealthStore : MonoBehaviour {
		...
		*/
		...
		/*! @brief 				Perform a statistics collection query.
			...
			@param handler		Called when the function finishes executing.
		 */
		public void ReadStatisticsCollection(HKDataType dataType, Predicate predicate, StatisticsOptions options, DateTimeOffset startDate, DateTimeOffset endDate, TimeSpan interval, ReceivedHealthData<HealthStatisticsCollection, Error> handler) {
			this.receivedStatisticsCollectionHandlers[dataType] = handler;
			this.ReadHealthStatisticsCollection(dataType, predicate, options, startDate, endDate, interval);
		}
...
#if UNITY_IOS && !UNITY_EDITOR
				_ReadStatisticsCollection(identifier, predicateString, optionsString, anchorStamp, null, intervalString);
#elif UNITY_EDITOR
		...
		private void ReadHealthStatisticsCollection(HKDataType dataType, Predicate predicate, StatisticsOptions options, DateTimeOffset startDate, DateTimeOffset endDate, TimeSpan interval)
		{
			if (this.IsHealthDataAvailable()) {
				string identifier = HealthKitDataTypes.GetIdentifier(dataType);
				string predicateString = (predicate != null) ? predicate.ToXMLString() : null;
				string optionsString = options.ToString();
				string startStamp = DateTimeBridge.DateToString(startDate);
				string endStamp = (endDate != null) ? DateTimeBridge.DateToString(endDate) : null;
				string intervalString = TimeSpanBridge.TimeSpanToXML(interval);
				//Debug.LogFormat("reading statistics collection from:\n-{0} ({1})\nto:\n-{2} ({3})", startDate, startStamp, endDate, endStamp);
#if UNITY_IOS && !UNITY_EDITOR
				_ReadStatisticsCollection(identifier, predicateString, optionsString, startStamp, endStamp, intervalString);
#elif UNITY_EDITOR
				this._ReadStatisticsCollection_DUMMY(identifier, predicateString, optionsString, startStamp, intervalString);
#endif
			}
			else {
				Debug.LogError("Error: no health data is available. Are you running on an iOS device that supports HealthKit?");
		...
		[DllImport ("__Internal")]
		private static extern void _ReadStatisticsCollection(string identifier, string predicateString, string optionsString, string anchorStamp, string endStamp, string intervalString);
