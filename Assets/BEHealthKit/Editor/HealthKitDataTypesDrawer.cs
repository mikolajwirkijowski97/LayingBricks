...
namespace BeliefEngine.HealthKit
{
	...
	public partial class HealthKitDataTypesEditor : Editor
	{
		...
		private bool otherQuantitiesSection = true;
		...
		public override void OnInspectorGUI() {
			...
			int otherQuantStart = Convert.ToInt32(HKDataType.HKQuantityTypeIdentifierUVExposure);
			...
			// OTHER
			int otherStart = Convert.ToInt32(HKDataType.HKDocumentTypeIdentifierCDA);
			int otherEnd = Convert.ToInt32(HKDataType.HKVisionPrescriptionTypeIdentifier);
			...
			if (mobilitySection) {
				DrawDataTypes<HKDataType>(mobilityStart, otherQuantStart - 1);
			}
			otherQuantitiesSection = EditorGUILayout.Foldout(otherQuantitiesSection, "Other Quantities");
			if (otherQuantitiesSection) {
				DrawDataTypes<HKDataType>(otherQuantStart, categoriesStart - 1);
			}
			...
			if (correlationSection) {
				DrawDataTypes<HKDataType>(corrStart, corrStart + 1);
			}
			otherSection = EditorGUILayout.Foldout(otherSection, "Other");
			if (otherSection) {
				DrawDataTypes<HKDataType>(otherStart, otherEnd);
			}
