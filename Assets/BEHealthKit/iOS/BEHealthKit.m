...
- (NSArray *)quantityTypes:(BOOL)write
{
	...
	if (@available(iOS 16.0, *)) {
		...
		[quantityTypes addObject:HKQuantityTypeIdentifierRunningStrideLength];
		[quantityTypes addObject:HKQuantityTypeIdentifierRunningVerticalOscillation];
		[quantityTypes addObject:HKQuantityTypeIdentifierRunningGroundContactTime];
		[quantityTypes addObject:HKQuantityTypeIdentifierRunningPower];
		[quantityTypes addObject:HKQuantityTypeIdentifierRunningSpeed];
		[quantityTypes addObject:HKQuantityTypeIdentifierHeartRateRecoveryOneMinute];
		[quantityTypes addObject:HKQuantityTypeIdentifierAtrialFibrillationBurden];
		[quantityTypes addObject:HKQuantityTypeIdentifierUnderwaterDepth];
		[quantityTypes addObject:HKQuantityTypeIdentifierWaterTemperature];
	}
...
//		NSLog(@"default unit for %@ = %@", identifier, [self defaultUnitForSampleType:[HKSampleType quantityTypeForIdentifier:identifier]]);
	...
	if (@available(iOS 15.0, *)) {
		checkCategoryType(HKCategoryTypeIdentifierAppleWalkingSteadinessEvent, identifiers, dataTypes);
	}
	if (@available(iOS 16.0, *)) {
		checkCategoryType(HKCategoryTypeIdentifierPersistentIntermenstrualBleeding, identifiers, dataTypes);
		checkCategoryType(HKCategoryTypeIdentifierProlongedMenstrualPeriods, identifiers, dataTypes);
		checkCategoryType(HKCategoryTypeIdentifierIrregularMenstrualCycles, identifiers, dataTypes);
		checkCategoryType(HKCategoryTypeIdentifierInfrequentMenstrualCycles, identifiers, dataTypes);
	}
	...
	/*-------------------------------*/
	/*   Series Sample Identifiers   */
	/*-------------------------------*/

	void (^checkSeriesSampleType)(NSString *, NSMutableArray *, NSMutableSet *) = ^void (NSString *identifier, NSMutableArray *allIdentifiers, NSMutableSet *dataTypes)
	{
		if ([allIdentifiers containsObject:identifier]) {
			[dataTypes addObject:[HKSeriesType seriesTypeForIdentifier:identifier]];
			[allIdentifiers removeObject:identifier];
		}
		
	};
	
	checkSeriesSampleType(HKWorkoutRouteTypeIdentifier, identifiers, dataTypes);
	if (@available(iOS 13.0, *)) {
		checkSeriesSampleType(HKDataTypeIdentifierHeartbeatSeries, identifiers, dataTypes);
	}

	/*
	// Vision Prescription
	if (@available(iOS 16.0, *)) {
		if ([identifiers containsObject:HKVisionPrescriptionTypeIdentifier]) {
			[dataTypes addObject:[HKSampleType visionPrescriptionType]];
			[identifiers removeObject:HKVisionPrescriptionTypeIdentifier];
		}
	}
	*/
