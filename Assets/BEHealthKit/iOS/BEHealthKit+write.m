...
void _BuildWorkout(char *builderString) {
	NSString *xml = [NSString stringWithCString:builderString encoding:NSUTF8StringEncoding];
	HKHealthStore *store = [BEHealthKit sharedHealthKit].healthStore;
	[HKWorkoutBuilder buildWorkoutFromXMLString:xml healthStore:store completion:^(BOOL success, HKWorkoutBuilder *builder, NSError *error) {
		NSMutableDictionary *responseDict = [@{@"success":@(success), XMLDictionaryNodeNameKey:@"builder"} mutableCopy];
		responseDict[@"datatype"] = HKWorkoutTypeIdentifier;
		int activityType = (int)builder.workoutConfiguration.activityType;
		responseDict[@"workoutType"] = @(activityType);
		if (error) {
			responseDict[@"error"] = [error dictionary];
		}
		NSString *response = [responseDict XMLString];
		NSLog(@"response:\n %@", response);
		UnitySendMessage([[BEHealthKit sharedHealthKit].controllerName cStringUsingEncoding:NSUTF8StringEncoding], "ParseHealthXML", [response cStringUsingEncoding:NSUTF8StringEncoding]);
	}];
...
void saveSampleWithCompletion(NSString *datatype, HKObject *sample, CompletionBlock completion) {
	...
	[kit.healthStore saveObject:sample withCompletion:^(BOOL success, NSError *error) {
		...
		dict[@"workoutType"] = datatype;
		if ([sample isKindOfClass:[HKWorkout class]]) {
			int workoutType = (int)[(HKWorkout *)sample workoutActivityType];
