...
void _ReadStatisticsCollection(char *identifier, char *predicateString, char *optionsString, char *startStamp, char *endStamp, char *intervalString) {
	NSString *identifierString = [NSString stringWithCString:identifier encoding:NSUTF8StringEncoding];
	HKQuantityType *sampleType = [HKSampleType quantityTypeForIdentifier:identifierString];
	if (!sampleType) {
		NSLog(@"Error; unknown quantity-type identifier '%@'", identifierString);
	...
	BEHealthKit *kit = [BEHealthKit sharedHealthKit];
	NSDate *anchorDate = [NSDate dateFromBridgeString:startStamp];
	...
	NSPredicate *spanPredicate = nil;
	if (endStamp != NULL) {
		NSDate *endDate = [NSDate dateFromBridgeString:endStamp];
		
		spanPredicate = [HKQuery predicateForSamplesWithStartDate:anchorDate endDate:endDate options:HKQueryOptionStrictStartDate];
	}
	if (predicate && spanPredicate) {
		predicate = [NSCompoundPredicate andPredicateWithSubpredicates: @[spanPredicate, predicate]];
	} else if (spanPredicate) {
		predicate = spanPredicate;
	}
