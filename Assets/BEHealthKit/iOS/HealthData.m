...
+ (NSString *)XMLFromSamples:(NSArray *)samples datatype:(HKObjectType *)datatype error:(NSError *)error
{
	...
	if ([datatype isKindOfClass:[HKWorkoutType class]]) {
		...
		return [HealthData XMLFromWorkoutSamples:samples workoutType:workoutType error:error];
	}
...
+ (NSString *)XMLFromWorkoutSamples:(NSArray *)workoutSamples workoutType:(HKWorkoutActivityType)workoutType error:(NSError *)error
{
	NSMutableDictionary *dict = [NSMutableDictionary dictionary];
